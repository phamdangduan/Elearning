...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Logout handler
    const logoutBtn = document.getElementById('logoutBtn') || document.getElementById('landingLogoutBtn');
...
function renderCourse(data) {
    ...
    // Show add to cart button if not enrolled
    const cartBtn = document.getElementById('addToCartBtn');
    if (cartBtn) {
        if (window.isEnrolled) {
            ...
            newCartBtn.addEventListener('click', () => {
                const cId = data.id || courseId;
                addToCart(cId, data.title, data.price || 0, thumbUrl || '', teacherName);
            });
        }
    }
}
...
async function enrollCourse(courseId, token) {
    ...
    window.location.href = `student/checkout.html?courseId=${courseId}`;
}
